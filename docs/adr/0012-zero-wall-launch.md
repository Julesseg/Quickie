...
- **Permissions are requested just-in-time**, at the first moment each is needed (EventKit when the user first **activates** a capture Action such as New Reminder — before any data entry, since the breadcrumb capture can't proceed without it; the document picker when first adding an Indexed Folder), each preceded by a **one-line custom primer** so the system dialog isn't wasted on an uninformed denial.
