...
An expressive dynamic backdrop (animated mesh gradient, blurred icons) was considered and rejected for v1 in favor of calm and speed. **Partially superseded by ADR 0034**: the backdrop is now a mesh gradient that drifts only on Home and stills whenever a query exists — the calm-in-use protection stands; the flat rejection does not. ADR 0034 also extends the budget discipline here to hint rotation, haptics, and the Highlighted result's subtle hero shimmer (#177).
