...
// Matcher, keeps only strong matches, and caps how many surface inline. These
...
struct FileSearchProviderTests {
    ...
    @Test("only strong (exact/prefix) matches surface inline; weak ones are held back")
    func weakMatchesHeldBack() {
        // A buried substring ("port" inside "report.pdf") scores below the
        // strong-match threshold, so it does not surface inline — it would only
        // appear in the uncapped Search Files context (ADR 0014, a later slice).
        let p = provider([FileEntry(bookmarkID: "f", relativePath: "report.pdf")])
        #expect(p.candidates(for: "port").isEmpty)
        // The prefix of the same file is strong, so it does surface.
