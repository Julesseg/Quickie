...
**Disabled**:
An [[Action]] or provider kind that has been **reversibly hidden** from every surface — typed [[Result list]] rows, the Frecency "Recent" list, and the [[Favorites grid]] — while its data and configuration are retained. Distinct from **delete** (which destroys): disable is the reversible off-switch, delete the permanent one, and both coexist for user content. It exists at two levels: **kind** (a provider's Enabled toggle, the first entry of its [[Management page]]'s options section) and **instance** (a single Quicklink / Snippet / [[Pile]] entry / Shortcut / Fallback, toggled in that page's actions section — generalizing the original per-Fallback disable). A disabled kind short-circuits its instances. Disabling a pinned action keeps the pin (restored on re-enable) but drops it from the grid without consuming a visible slot.
