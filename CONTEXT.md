...
**Secondary action**:
A one-shot action that operates on a specific result's [[Result content]], reached from the row's long-press menu — the *same* menu that carries Pin/Unpin, not a competing gesture. Eligibility is a pure function of the result's Result content, **not** its content type: a row with no content (a command or capture row) exposes none, even though it may carry a `text` output type (ADR 0017). The current set is `copy` and `share` on any value-bearing row (text, URL, number, or Pile entry) plus `revealInFiles` on a file and `edit` on a [[Snippet]] (open its editor — a stored, titled record, unlike a bare text value); a [[Shortcut Action]] exposes `edit` **alone** — a deeplink into the Shortcuts app's editor for the shortcut by name (`shortcuts://open-shortcut`), since it is a launchable reference with no text to copy or share; a [[Pile]] entry additionally exposes **Remove from Pile** (its home for per-row discard, since the main action stages rather than removes). Multi-step actions that need a target, date, or name (Make-reminder, Make-Quicklink) are deferred to a later breadcrumb-seeding slice. Distinct from Fallback/content-on-the-text actions, which appear as their own rows rather than behind long-press.
...
**Result content**:
The concrete value or reference a result carries — the thing a [[Secondary action]] operates on — distinct from both its **content type** (the *kind*) and its **main action** (what tapping it does). Either an inline value (a Snippet's or Calculator's text, a Quicklink's URL) or an edge-resolved reference the app dereferences on demand (a [[Pile]] entry's text by id, a File by bookmark + relative path, a [[Shortcut Action]] by name). A command or capture row has **no** Result content, which is what makes it ineligible for secondary actions regardless of its content type (ADR 0017). A Shortcut is the one reference carrying no textual value — its name is a pure handle for the **Edit** deeplink, so it is eligible for that verb alone (no copy/share). Declared per Action, not derived from the main-action outcome (a Shortcut's run outcome is empty yet it declares its name; a Calculator reads as `number` though it copies text).
_Avoid_: Payload, value, content (ambiguous with content type)

**Fallback Action**:
Any Action whose **fallback flag** is on: always surfaced in the result list's bottom fallback region, its point being to **consume the user's typed query as its first Argument** rather than to be found by name-matching. Selecting one **seeds-and-commits**: the query text is committed as Argument 1 through the normal breadcrumb engine — a single-Argument fallback (web search, Save for later, New Snippet) therefore completes in one tap as today, while a multi-Argument one continues the breadcrumb at step 2 with pill 1 sealed (re-editable like any pill). The flag is orthogonal to what the Action is — a fallback-flagged [[Custom Action]] is still startable verb-first, where the breadcrumb begins empty — and is only available when Argument 1 is free text (a date- or choice-first Action has nowhere to put the query). The single result list interleaves fallbacks with name matches; the user resolves intent by choosing a row, never by a mode toggle. Fallbacks live in one user-ordered, reversible list (see Fallback list) and each can be **disabled** (hidden from results) without being deleted.
_Avoid_: Default action, catch-all

**Quicklink**:
A stored *static* URL that opens directly in the user's system-default browser — no `{slot}` token, no typed text consumed (Indexed Provider). It matches by name/alias like any other Action. Templated, slot-bearing links are a separate concept — see [[Custom Action]]. Quickie ships **no default Quicklinks**; the user adds their own (via the Share Extension or the Quicklinks page).
_Avoid_: Bookmark, link, URL action, template (a Quicklink has no template)

**Custom Action**:
A user-authored Action defined by a **URL template**: a stored URL that **requires** at least one `{slot}` token, whose slots become the breadcrumb's ordered, typed [[Argument]]s, and whose final commit opens the filled URL (e.g. `things:///add?title={…}&when={…}`). It matches by name/alias like any Action — verb-first selection starts the breadcrumb empty at Argument 1 — and may additionally carry the fallback flag (see [[Fallback Action]]) so the typed query seeds its first Argument. Because submission opens a URL it hands off to another app or the browser, so it is **not** a [[Quick capture]]. Web search is a default-seeded one-Argument, fallback-flagged Custom Action (a normal, fully deletable entry); the former "Fallback query" kind is absorbed wholesale into this concept. Distinct from a [[Quicklink]], which is static and slot-less.
...
**Fallback list**:
The single user-ordered list of every Fallback Action (fallback-flagged [[Custom Action]]s + Save for later + New Snippet), managed on one page and persisted as an explicit order. It reads **most-important-first**: the top of the page is the fallback nearest the input/thumb in results. Because the Result list renders reversed, this page order is reversed when projected into the bottom (screen-top) fallback region. Each row can be **disabled** (kept in the list, hidden from results); rows can be reordered; only Custom Actions can be deleted, while Save for later and New Snippet are permanent (disable-only). It orders and disables fallbacks; creating/editing a Custom Action happens on the Custom Actions [[Management page]]. Replaces the previous alphabetical fallback ordering.
